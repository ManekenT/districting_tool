...
import { algorithms } from "../../app/page";
...
export function AlgorithmStep(props: Props) {
    return (
        <Step
            finished={props.algorithm !== undefined}
            stepIndex={2}
            title="Algorithmus wählen"
            helpText={<AlgorithmsHelp />}
        >
...
function AlgoOption(algo: Algorithm) {
    return (
        <div className="flex items-center gap-x-2" key={algo.name}>
            <RadioGroup.Option
                value={algo}
                key={algo.name}
            ...
            <InfoModal>{algo.helpText}</InfoModal>
        </div>
    );
}
